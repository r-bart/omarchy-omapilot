...
Scope {
  ...
  // every one of those messages went nowhere. The panel renders this.
...
// Every refusal below leaves an action already on screen untouched. A hotkey
  ...
  // arrives before there is anywhere to show it.
...
if (type === "selection") {
      ...
      if (event.available !== true) {
        ...
        note(Protocol.selectionUnavailableMessage(unavailableReason))
      ...
      if (selectionText === "") {
        selectionTarget = ""
        note(Protocol.selectionUnavailableMessage("empty"))
    ...
    if (type === "selection_replaced") {
      ...
      if (event.replaced === true) {
        endTextAction()
        note("Replaced")
      } else note(Protocol.selectionFailureMessage(event.reason))
      return
    }
    if (type === "copied") {
