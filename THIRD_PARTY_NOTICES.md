...
The exact transitive dependency graph and declared licenses for each runtime archive are recorded in its generated CycloneDX `sbom.cdx.json`. The broker's complete bundled license inventory is shipped as `runtime/dist/quickchat-broker.THIRD_PARTY_LICENSES.txt`; adapter bundle legal notices are shipped beside their bundles. This notice is attribution, not a replacement for those license texts.
